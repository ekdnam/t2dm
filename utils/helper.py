...
class MyDriver:
	def __init__(self, folder_name, **kwargs):
		...
		self.driver = webdriver.Chrome(self.__CHROMEDRIVERPATH__, chrome_options=self.options)
	...
	def download(self, pmids: list):
	    ...
	    for pmid in pmids:
	        ...
	        # rename files
	        MyDriver.rename_files(pdf_files[0], str(pmid), "insulin_mortality_odds_ratio")
